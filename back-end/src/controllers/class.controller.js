...
export const createMaterial = async(req,res)=>{
  ...
  try {
    ...
    console.log("ErrorInCreateMaterialController ",error.message);
    res.status(500).json({message:"Internal Server Error"});
  }
}
...
export const deleteMaterial = async (req, res) => {
  ...
  try {
    ...
    const material = classRoom.material.id(materialId);
    ...
    await deleteFromCloudinary(material.materialName, material.type);

    
    classRoom.material.pull({ _id: materialId });
    ...
    console.log("ErrorInDeleteMaterial", error.message);
    res.status(500).json({ message: "Internal Server Error" });
  }
};
