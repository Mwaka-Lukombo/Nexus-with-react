...
export const MyClassesPages = () => {
  ...
  if(userAuth?.typeUser === 'teacher'){
     myClasses = classRooms?.filter((prev) => prev.teacherId?._id === userAuth?._id);
  }else{
    myClasses = classRooms?.filter((prev) => 
    prev?.course === userAuth?.course && prev?.year === Number(userAuth?.year)
    )
  }
