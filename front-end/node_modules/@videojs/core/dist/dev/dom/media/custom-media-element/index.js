...
function CustomMediaMixin(superclass, { tag, is }) {
	...
	return class CustomMedia extends superclass {
		...
		init() {
			...
			for (const type of this.constructor.Events) this.shadowRoot.addEventListener(type, this.#deferForwardEvent, true);
		}
		#deferForwardEvent = (event) => {
			if (this.target && this.target === event.target) this.target.addEventListener(event.type, this.#forwardEvent, { once: true });
		};
		#forwardEvent = (event) => {
